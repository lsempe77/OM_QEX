...
def main():
    ...
    # Use the actual page number where Table 17 lives
    # If your viewer says "page 45", use "45" here
    tables = camelot.read_pdf(str(pdf_path), pages="47", flavor="stream")

    print(f"Found {len(tables)} tables on page 47")
    for i, t in enumerate(tables):
        first_row = t.df.iloc[0].tolist()
        print(f"[stream] Table {i}: shape={t.df.shape}, first_row={first_row}")

        out_csv = out_dir / f"teep_table17_stream_{i}.csv"
